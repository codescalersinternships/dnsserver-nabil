use std::{fs::File, io::{self, Error, Read}, net::{Ipv4Addr, Ipv6Addr, UdpSocket}};
...
impl BytePacketBuffer {
    ...
    fn get(& self, pos: usize) -> Result<u8, io::Error> {
        if pos >= 512 {
            return Err(Error::new(io::ErrorKind::UnexpectedEof, "End of buffer"));
        }
        Ok(self.buf[pos])
    }
    fn get_range(& self, start: usize, len: usize) -> Result<&[u8], io::Error> {
        if start+len >= 512 {
            return Err(Error::new(io::ErrorKind::UnexpectedEof, "End of buffer"));
        }
...
fn handle_query(socket: &UdpSocket) -> Result<(), io::Error> {
    let mut req_buffer = BytePacketBuffer::new();
